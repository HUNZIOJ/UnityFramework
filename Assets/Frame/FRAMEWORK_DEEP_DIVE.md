...
- 业务层只依赖接口，例如 `IUIService`、`ISaveService`、`IHttpService`，降低替换实现的成本。
...
| `OnGUI()` | 绘制 IMGUI 面板 | visible false 直接返回；计算宽度；创建窗口和 scroll view；依次绘制 Runtime、Lifecycle、HTTP、Timers、Scenes、Assets、Pools、Logs | 使用 IMGUI，主要面向调试，不是正式 UI |
...
| `RefreshServices()` | 重新解析可选服务 | 对 diagnostics、lifecycle、http、assets、pools、scenes、timers 分别 `Framework.TryResolve(out ...)` | 模块关闭或未启用时保持 null，绘制层显示不可用 |
...
`ConfigService` 默认注册两个 Provider：

1. `ResourcesScriptableConfigProvider`：读取 `Resources/Configs` 下的 `ScriptableConfig`。
2. `ResourcesJsonConfigProvider`：读取 `Resources/Configs` 下的 JSON TextAsset。
...
| `ResourcesJsonConfigProvider` | Resources JSON Provider | 从 `Resources/Configs` 加载 TextAsset 并 Newtonsoft 反序列化 |
...
| `ResourcesScriptableConfigProvider` | Resources Scriptable Provider | 初始化时 LoadAll `Resources/Configs` 下的 ScriptableConfig |
...
| 成员 | 作用 | 实现方式 | 使用方式和注意点 |
| --- | --- | --- | --- |
| `ResourcesScriptableConfigProvider(string rootPath = "Configs")` | 从 Resources 预加载 ScriptableConfig | 规范化 rootPath；`Resources.LoadAll<ScriptableObject>(path)`；筛选 `ScriptableConfig` 并注册到内部 provider | 默认读取 `Resources/Configs` 下所有 ScriptableConfig |
| `TryLoad<TConfig>(string key, out TConfig config)` | 查找 Scriptable 配置 | 转发给内部 `ScriptableConfigProvider` | 构造时已经完成 LoadAll，之后不会自动刷新 |

#### `ResourcesJsonConfigProvider.cs`

| 成员 | 作用 | 实现方式 | 使用方式和注意点 |
| --- | --- | --- | --- |
| `ResourcesJsonConfigProvider(string rootPath = "Configs")` | 创建 JSON Provider | 保存规范化 rootPath | 默认读取 `Resources/Configs` |
| `TryLoad<TConfig>(string key, out TConfig config)` | 加载 JSON 配置 | 拼接 rootPath/key 并规范化；`Resources.Load<TextAsset>`；找不到返回 false；找到后用 `JsonConvert.DeserializeObject<TConfig>`；异常写 `FrameLog.Exception` 并返回 false | key 不需要写 `.json` 扩展名 |
...
| `SetJson(string key, string json)` | 设置原始 JSON | key 规范化后为空抛异常；json 为空则 Remove；否则写入字典并 RaiseChanged | 支持运行时覆盖 Resources 配置 |
...
| `NormalizeKey(string key)` | 规范化 key | 调 `FramePathUtility.NormalizeResourcesPath(key)` | 与 Resources 配置路径规则保持一致 |
...
| `OnInitialize()` | 初始化 Provider 链 | 添加 `ResourcesScriptableConfigProvider` 和 `ResourcesJsonConfigProvider`，注册 `IConfigService` 和自身实现 | 默认 Scriptable Provider 优先于 JSON Provider |
...
Networking 模块封装 UnityWebRequest，提供 BaseUrl、默认 Header、Bearer Token、JSON 请求、重试、取消、统计事件和响应解析器。
...
这一节按 `Assets/Frame/Runtime/Networking` 的实际源码文件展开。Networking 模块用 `UnityWebRequest` 发请求，用 UniTask 驱动异步，外层用 `HttpRequestHandle` 给 coroutine 和取消流程提供统一句柄。
...
table.SetLocales("en", "zh");
table.SetValue("hello", "en", "Hello {0}");
table.SetValue("hello", "zh", "你好 {0}");
```
...
`LocalizedTextTable` 支持两种数据源：

- 手动序列化 entries 和 locales。
- 绑定 CSV/TSV TextAsset，第一列为 key，后续列为 locale。
...
| `LocalizedTextTable` | 本地化文本表 | ScriptableObject，支持手动 entries、CSV、TSV、TextAsset 源、字典缓存 |
| `LocalizedText` | UI 文本绑定组件 | 绑定 `UnityEngine.UI.Text`，语言变化时刷新 |
| `Entry` | `LocalizedTextTable` 私有行数据 | 保存一个 key 和每个 locale 对应的 value 列表 |
...
| `Clear()` | 清空表数据 | source 置 null，locales/entries 清空，MarkDirty | 不销毁资产 |
| `SetLocales(params string[] localeCodes)` | 设置语言列 | 清空 locales；逐个 Clean 并 AddLocaleIfMissing；NormalizeEntryValueCounts；MarkDirty | 会保留 entries，但调整 value 列数量 |
| `SetValue(string key, string locale, string value)` | 设置单个翻译 | Clean key/locale；空则返回；确保 locale 存在；FindOrCreateEntry；扩展 Values；写入 value 或空字符串；MarkDirty | 适合编辑器工具或运行时构建表 |
...
| `ImportDelimitedText(string text, char textDelimiter)` | 导入分隔文本 | 清 source、设置 delimiter、清 locales/entries；ParseDelimited；ApplyRows；MarkDirty | 会把文本内容转成序列化 entries |
| `SetSource(TextAsset textAsset, string textDelimiter = ",")` | 绑定外部文本源 | 保存 source 和 delimiter，MarkDirty | source 优先级高于手动 entries |
| `OnEnable()` | 启用时标脏 | `lookupDirty = true` | 资产加载后延迟重建 lookup |
| `OnValidate()` | 编辑器校验 | `lookupDirty = true` | Inspector 改动后刷新缓存 |
| `EnsureLookup()` | 确保查询字典有效 | 如果不脏且 lookup 非空返回；否则创建或清空 lookup，清 availableLocales；有 source 文本则 ParseDelimited 后 BuildLookupFromRows，否则 BuildLookupFromSerializedEntries；最后 lookupDirty false | 所有查询入口都会调用 |
| `BuildLookupFromSerializedEntries()` | 从手动 entries 建 lookup | 遍历 entries 和 locales；跳过空 key/locale；按 entry.Values 写入 lookup | 序列化数据源路径 |
| `BuildLookupFromRows(List<List<string>> rows)` | 从 source 行建 lookup | 第一行是 header，第一列 key，后续列 locale；逐行读取 key 和各语言 value，AddLookupValue | 运行时绑定 TextAsset 时使用 |
| `ApplyRows(List<List<string>> rows)` | 把导入行写入序列化字段 | 读取 header 语言；逐行 FindOrCreateEntry 并写 Values | ImportCsv/ImportTsv 使用 |
...
| `AddLocaleIfMissing(string locale)` | 添加语言列 | 空返回 -1；已有返回索引；否则 Add 并返回新索引 | SetLocales/SetValue/ApplyRows 共用 |
| `FindOrCreateEntry(string key)` | 查找或创建行 | 线性查 entries 中 Key 相等项；找不到则 new Entry 并 Add | 手动 entries 数量大时是线性成本 |
| `NormalizeEntryValueCounts()` | 对齐每行 value 数量 | 遍历 entries，非空则 EnsureValueCount 到 locales.Count | 改语言列后避免越界 |
| `EnsureValueCount(List<string> values, int count)` | 扩展列表长度 | while 不足 count 就 Add 空字符串 | 不会截断多余 value |
...
| `Entry` | 私有行数据 | `Key` 字符串加 `Values` 列表 | Values 索引与 locales 索引对应 |

## StateMachine 模块

StateMachine 是通用状态机工具，不是框架服务模块。它适合角色状态、流程状态、UI 状态等轻量场景，
同时具备生产级能力：守卫、数据驱动转换、携带数据切换、历史回退、防重入、异常隔离。
...
// enum id 建议显式传 EqualityComparer 以避免字典查找装箱
StateMachine<BattleStateId> machine = new StateMachine<BattleStateId>(
    System.Collections.Generic.EqualityComparer<BattleStateId>.Default);

machine.Add(new PrepareState());           // 按 state.Id 注册
machine.Add(new FightingState());

// 数据驱动转换：每次 Tick 自动评估条件
machine.AddTransition(BattleStateId.Prepare, BattleStateId.Fighting, () => countdown <= 0);
machine.AddAnyTransition(BattleStateId.Result, () => allEnemiesDead);

machine.StateChanged += (from, to) => FrameLog.Info($"FSM {from} -> {to}");

machine.Change(BattleStateId.Prepare);
machine.Tick(Time.deltaTime);              // 在 Update 里调用
machine.Change(BattleStateId.Fighting, payload: enemyList);  // 携带数据切换
```

### 设计和实现

`StateMachine<TStateId>` 使用字典保存状态。`Change(id[, payload])` 时：先校验 id 存在（strict 模式下未知 id 抛
`FrameStateMachineException`，否则返回 false）；询问当前状态的 `IStateGuard.CanExit` 与目标状态的
`IStateGuard.CanEnter`，任一否决则中止；通过后调用旧状态 `Exit()`、切换引用、调用新状态 `Enter()`，
若目标实现 `IPayloadState` 再调 `OnEnterWithPayload(payload)`；最后触发 `StateChanged` 事件。

若在 `Enter/Exit` 内部再次调用 `Change`，请求会被放入队列、在当前转换结束后依次应用（防止递归 Exit/Enter 栈）。
`Tick(deltaTime)` 先评估所有数据驱动转换（命中即切换并 return，下一帧从新状态继续），再转发给当前状态。
所有状态回调（Enter/Tick/Exit/payload/转换条件/StateChanged）抛出的异常都会被包成 `FrameStateMachineException`
（保留 `InnerException`）抛出，避免被静默吞掉。`FixedTick/LateTick` 仅转发给实现了对应能力接口的当前状态。

### 类型职责

| 类型 | 作用 | 关键点 |
| --- | --- | --- |
| `IState<TStateId>` | 状态基础接口 | Id、Enter、Tick、Exit |
| `StateBase<TStateId>` | 可选便利基类 | 空虚方法 + `Machine` 反向引用，只重写需要的钩子 |
| `IPayloadState` | 可选：接收切换时传入的 payload | `OnEnterWithPayload(object)` |
| `IStateGuard<TStateId>` | 可选：否决转换 | `CanExit(to)` / `CanEnter(from)` |
| `IFixedTickState` / `ILateTickState` | 可选：物理帧/后期帧更新 | `FixedTick` / `LateTick` |
| `StateTransition<TStateId>` | 数据驱动转换规则 | From/To/Condition/FromAny |
| `StateMachine<TStateId>` | 状态机实现 | Add、Change、Tick、Clear + 转换表/守卫/历史/事件 |
| `FrameStateMachineException` | 状态机异常 | 包裹状态回调异常，保留 InnerException |
...
| 成员 | 作用 | 实现方式 | 使用方式和注意点 |
| --- | --- | --- | --- |
| `Id` | 状态唯一标识 | 状态实现类返回泛型 id | `StateMachine<TStateId>` 用它作为字典 key |
| `Enter()` | 进入状态 | 状态机切换到该状态后调用 | 初始化动画、UI、计时器可放这里 |
| `Tick(float deltaTime)` | 状态更新 | 状态机 `Tick` 时转发给当前状态 | 调用方决定 deltaTime 来源 |
| `Exit()` | 退出状态 | 状态机离开当前状态或 Clear 时调用 | 释放状态内临时资源、取消事件订阅 |

#### `StateCapabilities.cs` / `StateBase.cs` / `StateTransition.cs`（可选能力）

| 成员 | 作用 | 使用方式和注意点 |
| --- | --- | --- |
| `IPayloadState.OnEnterWithPayload(object)` | 接收 `Change(id, payload)` 的数据 | `Enter()` 之后立即调用一次；不实现则忽略 payload |
...
| `IStateGuard<TStateId>.CanExit/CanEnter` | 否决转换 | 返回 false 则 `Change` 返回 false、状态不变 |
| `StateBase<TStateId>` | 便利基类 | 提供空虚方法与 `Machine` 反向引用（`Add` 时自动注入），便于状态内部请求切换 |
| `StateTransition<TStateId>` | 转换规则对象 | 由 `AddTransition/AddAnyTransition` 构造，`Condition` 为空抛 `ArgumentNullException` |

#### `StateMachine.cs`

| 成员 | 作用 | 实现方式 | 使用方式和注意点 |
| --- | --- | --- | --- |
| `StateMachine()` / `StateMachine(comparer, strict)` | 构造 | 默认用 `EqualityComparer<TStateId>.Default`；可传自定义 comparer 与严格模式 | enum id 显式传 comparer 可避免装箱 |
| `CurrentState` | 当前状态对象 | `Change()` 成功时设置，`Clear()` 时清空 | 可为空 |
| `CurrentId` | 当前状态 id | 当前状态为空返回 `default`，否则返回 `CurrentState.Id` | enum 的 default 可能也是合法值，判断是否运行看 `IsRunning`/`CurrentState` |
| `IsRunning` | 是否有当前状态 | `CurrentState != null` | — |
| `IsTransitioning` | 是否正在切换 | 进入 Exit/Enter 期间为 true | 此期间的 `Change` 会被排队 |
| `Strict` | 严格模式开关 | 可运行时修改 | true 时未知 id 抛 `FrameStateMachineException` |
| `Add(state)` | 添加或替换状态 | state 为空抛 `ArgumentNullException`；按 `state.Id` 写入；若为 `StateBase` 则注入 `Machine` | 同 id 状态会被覆盖 |
| `Has(id)` / `TryGetState(id, out)` | 查询状态 | 字典查找 | — |
| `AddTransition(from, to, condition)` | 注册条件转换 | 加入转换表，`Tick` 时按注册顺序评估 | condition 为空抛异常 |
| `AddAnyTransition(to, condition)` | 注册任意态转换 | `FromAny=true` 的转换 | 适合 Death/Pause 等全局切换 |
| `Change(id)` / `Change(id, payload)` | 切换状态 | 校验 id → 守卫 → Exit → 切换 → Enter → payload → StateChanged；转换中则入队 | 守卫否决/未知 id 返回 false（strict 抛异常）；切到当前状态返回 true 不重入 |
| `RevertToPrevious()` | 回退上一状态 | 记录上一个 id，无历史返回 false | 暂停菜单返回等 |
| `StateChanged` 事件 | 转换完成通知 (from, to) | 异常被包裹抛出 | `Clear()` 时置空，避免悬挂订阅 |
| `Tick(float)` | 更新当前状态 | 先评估转换表，再 `CurrentState.Tick` | 状态机不自动接入 Unity Update |
| `FixedTick(float)` / `LateTick(float)` | 物理/后期更新 | 仅当前状态实现对应接口时转发 | 由调用方在 FixedUpdate/LateUpdate 调用 |
| `Clear()` | 清空状态机 | 当前状态先 `Exit()`；清空状态、转换表、历史、待处理队列、事件 | 适合对象销毁或流程重置 |
...
7. 网络层统一配置 `BaseUrl`、Headers、`ResponseParser`。
...
如果后端 envelope 字段不是 `success/code/message/data`，可以直接配置 `EnvelopeHttpResponseParser` 字段；如果协议更复杂，实现新的 `IHttpResponseParser` 并赋值给 `HttpService.ResponseParser`。
