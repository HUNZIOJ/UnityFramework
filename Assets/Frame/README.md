...
- `Config`：支持 Resources JSON、ScriptableObject provider、运行时 JSON 覆盖、读取缓存、provider 优先级和 `IConfigValidator` 校验。
...
- `Networking`：基于 `UnityWebRequest` 的 GET/POST/PUT/DELETE，支持 base URL、默认 header、Bearer token、typed JSON、统一响应解析器、timeout、retry、取消和请求指标。
...
运行时排查可在 `FrameSettings` 中打开 Runtime Diagnostics Overlay，默认用反引号键切换显示，面板会展示 FPS/内存、生命周期状态、HTTP 指标、计时器数量、场景加载状态、资源引用计数、对象池统计和最近日志。
