...
> 本文档（`FRAMEWORK_DEEP_DIVE_claude.md`）面向准备阅读源码、二次开发、或在真实项目中深度使用 `Assets/Frame` 框架的工程师。它在 `README.md`、`FRAMEWORK_GUIDE.md` 的基础上进一步下沉到**每个文件、每个类型、每个方法与属性**的实现细节，并补充了大量**流转逻辑（control flow）**、**设计意图（why）** 与**踩坑点（gotchas）**。
>
...
- 业务层应**优先依赖接口**（`IUIService`、`ISaveService`、`IHttpService` 等），只有确实需要实现类特有能力时才解析具体类型，以降低替换实现的成本。
...
| Networking | `HttpService` | **0** | HTTP 请求 |
| Localization | `LocalizationService` | **0** | 本地化文本 |

> **注意**：Priority 相同的模块（Input / Networking / Localization 都是 0），其相对顺序取决于 `List.Sort` 的实现（非稳定排序）以及它们加入 `ModuleManager` 的顺序。业务**不应**依赖同优先级模块之间的相对初始化顺序。如果模块 A 在 `OnInitialize()` 中需要解析模块 B，应保证 B 的 Priority 严格小于 A。
...
| `RuntimeDiagnosticsOverlay` | 屏幕上的 IMGUI 诊断叠加面板（`MonoBehaviour`） | 按键切换显隐、定时刷新快照、聚合展示 7 个子系统状态 |
...
| `private IDiagnosticsService diagnostics` 等 7 个服务字段 | 缓存解析到的子系统服务 | `diagnostics/lifecycle/http/assets/pools/scenes/timers`，全部通过 `Framework.TryResolve` 填充 |
...
| `private void OnGUI()` | 绘制面板 | `!visible` 直接返回；`panelWidth = Mathf.Clamp(width, 280f, Mathf.Max(280f, Screen.width-20f))`；在窗口区域内依次调用 `DrawSnapshot/DrawLifecycle/DrawHttp/DrawTimers/DrawScenes/DrawAssets/DrawPools/DrawLogs`（注意 `OnGUI` 内调用顺序与方法定义顺序不同） |
...
| `private void RefreshServices()` | 重新解析全部服务 | 连续 `Framework.TryResolve(out ...)` 7 次 |
...
4. Overlay 数据流：`RuntimeDiagnosticsOverlay` 不直接监听日志，而是定时（默认每 0.25s，受 `nextRefreshTime` 节流）调用 `RefreshSnapshot`：`Framework.TryResolve` 重新解析 7 个服务，`diagnostics.CaptureSnapshot()` 拿快照，`assets.GetLoadedAssetStats()`/`pools.GetAllGameObjectPoolStats()` 拿列表；`OnGUI` 把这些缓存数据画出来，日志区则实时从 `diagnostics.Logs` 取最后 `maxLogLines` 行。任一服务解析不到时，对应区块显示 "... is not available." 而不会抛异常。
...
StateMachine 模块是一个泛型有限状态机实现。它用泛型参数 `TStateId` 标识状态（可用枚举、字符串、int 等任意类型），通过 `IState<TStateId>` 接口约定每个状态的 `Enter/Tick/Exit` 生命周期，由 `StateMachine<TStateId>` 负责注册状态、按 id 切换、转发 Tick 与整体清理。它不依赖 Unity 或框架其他模块，可独立用于 AI、UI 流程、游戏阶段管理等场景。在保留最小核心 API 的前提下，模块提供了一组**可选叠加**的生产级能力：携带数据切换、转换守卫、数据驱动转换、任意态转换、历史回退、防重入、物理/后期帧更新、转换事件、异常隔离与严格模式。

### 类型总览

| 类型 | 作用 | 关键点 |
| --- | --- | --- |
| `IState<TStateId>`（interface） | 单个状态的生命周期契约 | `Id` 只读；`Enter/Tick(float)/Exit` |
| `StateBase<TStateId>`（abstract class） | 可选便利基类 | 空虚方法 + `Machine` 反向引用 |
| `IPayloadState`（interface） | 可选：接收切换 payload | `OnEnterWithPayload(object)` |
| `IStateGuard<TStateId>`（interface） | 可选：否决转换 | `CanExit(to)`/`CanEnter(from)` |
| `IFixedTickState`/`ILateTickState`（interface） | 可选：物理/后期帧更新 | `FixedTick`/`LateTick` |
| `StateTransition<TStateId>`（sealed class） | 数据驱动转换规则 | From/To/Condition/FromAny |
| `StateMachine<TStateId>`（sealed class） | 状态机本体 | 字典存状态 + 转换表/守卫/历史/事件队列 |
| `FrameStateMachineException`（sealed class） | 状态机异常 | 包裹状态回调异常，保留 InnerException |

### `IState.cs`

| 成员 | 作用 | 实现/注意点 |
| --- | --- | --- |
| `interface IState<TStateId>` | 状态接口 | 无类型约束 |
| `TStateId Id { get; }` | 唯一标识 | 只读属性，作为字典 key |
| `void Enter()` | 进入状态 | — |
| `void Tick(float deltaTime)` | 每帧/每步驱动 | — |
| `void Exit()` | 离开状态 | — |

### `StateCapabilities.cs` / `StateBase.cs` / `StateTransition.cs`

| 成员 | 作用 | 实现/注意点 |
| --- | --- | --- |
| `IPayloadState.OnEnterWithPayload(object)` | 接收 `Change(id, payload)` 的数据 | `Enter()` 后立即调用一次；状态不实现则 payload 被忽略 |
| `IFixedTickState.FixedTick(float)` | 物理步更新 | 需调用方从 `FixedUpdate` 转发 `machine.FixedTick` |
| `ILateTickState.LateTick(float)` | 后期帧更新 | 需调用方从 `LateUpdate` 转发 `machine.LateTick` |
| `IStateGuard<TStateId>.CanExit(to)` | 当前状态能否离开 | 返回 false 中止转换 |
| `IStateGuard<TStateId>.CanEnter(from)` | 目标状态能否进入 | 返回 false 中止转换 |
| `abstract StateBase<TStateId>` | 便利基类 | 实现 `IState`，空虚 `Enter/Tick/Exit`，含 `Machine` 反向引用（`Add` 时由机器注入）便于状态内部 `Machine.Change(...)` |
| `sealed StateTransition<TStateId>` | 转换规则 | 字段 From/To/Condition/FromAny；`condition == null` 抛 `ArgumentNullException` |

### `StateMachine.cs`

| 成员 | 作用 | 实现/注意点 |
| --- | --- | --- |
| `StateMachine()` | 默认构造 | 用 `EqualityComparer<TStateId>.Default` |
| `StateMachine(IEqualityComparer<TStateId> comparer, bool strict=false)` | 自定义构造 | comparer 为 null 回退默认；enum id 显式传可避免装箱；`strict` 控制未知 id 行为 |
| `private readonly Dictionary<TStateId, IState<TStateId>> states` | id→状态 映射 | 用构造传入的 comparer |
| `private readonly List<StateTransition<TStateId>> transitions` | 数据驱动转换表 | 每次 Tick 按注册顺序评估 |
| `private readonly Queue<PendingChange> pendingChanges` | 转换中被推迟的切换请求 | 防重入用 |
| `event Action<TStateId,TStateId> StateChanged` | 转换完成事件 (from,to) | handler 异常被包裹抛出；`Clear()` 置空 |
| `CurrentState` / `CurrentId` | 当前状态 / id | 无状态时 id 返回 `default(TStateId)` |
| `IsRunning` | 是否有当前状态 | `CurrentState != null` |
| `IsTransitioning` | 是否正在 Exit/Enter | 该期间 `Change` 入队 |
| `Strict` | 严格模式开关 | 可运行时改 |
| `StateCount` | 已注册状态数 | — |
| `Add(state)` | 注册状态 | `null` 抛异常；`state.Id` 为 key 覆盖写入；若是 `StateBase` 注入 `Machine` |
| `Has(id)` / `TryGetState(id, out)` | 查询 | 字典查找 |
| `AddTransition(from, to, condition)` | 注册条件转换 | 加入转换表 |
| `AddAnyTransition(to, condition)` | 注册任意态转换 | `FromAny=true` |
| `Change(id)` | 切换（无 payload） | 转发 `Change(id, null)` |
| `Change(id, payload)` | 切换（带 payload） | 见下方流转逻辑 |
| `RevertToPrevious()` | 回退上一状态 | 无历史返回 false |
| `HasPrevious` | 是否有历史 | — |
| `Tick(float)` | 驱动 | 先 `EvaluateTransitions()` 再 `CurrentState.Tick` |
| `FixedTick(float)` / `LateTick(float)` | 物理/后期更新 | 仅当前状态实现对应接口时转发，异常包裹抛出 |
| `Clear()` | 清空 | 当前状态 `Exit()`；清空状态/转换表/历史/队列/事件 |

### 流转逻辑

- **注册**：`Add` 以 `Id` 为 key，重复 id 覆盖，`null` 抛异常；若状态继承 `StateBase` 则把 `Machine` 反向引用注入它。
- **切换（Change(id, payload)）**：
  1. `states.TryGetValue` 查目标；查不到时——`Strict` 为 true 抛 `FrameStateMachineException`，否则返回 `false`。
  2. 若 `IsTransitioning`（正处于别的 Exit/Enter 内），把请求 `Enqueue` 到 `pendingChanges` 并返回 `true`，**不**递归执行。
  3. 否则进入 `ApplyChange`：若目标已是当前状态返回 `true` 不重入；询问当前状态 `IStateGuard.CanExit(next.Id)`、目标 `IStateGuard.CanEnter(fromId)`，任一否决返回 `false`。
  4. 置 `IsTransitioning=true`，顺序执行「旧.Exit → 记录 previousId → 切引用 → 新.Enter →（实现 `IPayloadState` 则）OnEnterWithPayload」，最后 `finally` 复位 `IsTransitioning`。
  5. 触发 `StateChanged(from,to)`，再 `DrainPendingChanges()` 依次应用排队的切换。
- **数据驱动转换（Tick 内 EvaluateTransitions）**：按注册顺序遍历转换表，源匹配（`FromAny` 或 `From==CurrentId`）且 `Condition()` 为真且目标非当前，则 `Change(to)` 并 return（下一帧从新状态继续评估）。
- **异常隔离**：Enter/Tick/Exit/payload/转换条件/StateChanged 抛出的异常统一被 `Wrap` 成 `FrameStateMachineException`（保留 InnerException）抛出，不会被静默吞掉。
- **清理（Clear）**：当前状态先 `Exit`，再清空状态字典、转换表、历史、待处理队列并置空事件；清理后需重新 `Add` 才能再用。
...
public sealed class IdleState : StateBase<AiState>
{
    public override AiState Id => AiState.Idle;
    public override void Enter() { /* 播放待机动画 */ }
    public override void Tick(float dt) { /* 检测玩家 */ }
}

// enum id 显式传 comparer 避免装箱；strict 下未知 id 抛异常
var fsm = new StateMachine<AiState>(EqualityComparer<AiState>.Default);
...
// 数据驱动 + 任意态转换
fsm.AddTransition(AiState.Idle, AiState.Chase, () => sawPlayer);
fsm.AddAnyTransition(AiState.Dead, () => hp <= 0);

fsm.StateChanged += (from, to) => FrameLog.Info($"{from} -> {to}");

fsm.Change(AiState.Idle);                 // 首次进入：仅调用 IdleState.Enter
void Update() => fsm.Tick(Time.deltaTime);// 转发 Tick 并评估转换表

fsm.Change(AiState.Chase, payload: target);// 携带数据切换（ChaseState 实现 IPayloadState 即可收到）
fsm.RevertToPrevious();                     // 回到 Idle
fsm.Clear();                                // 当前状态 Exit，并清空所有注册
```

### 设计意图与踩坑点

- **向后兼容**：最小核心 API（`Add/Change/Tick/Clear` + `IState.Enter/Tick/Exit`）行为不变，旧代码与旧测试无需改动；新能力全部是叠加的可选接口。
- **自切换被短路**：对当前状态再次 `Change` 返回 `true` 但不触发 `Exit`/`Enter`，不能用它来「重置」状态。
- **防重入而非禁止重入**：在 `Enter/Exit` 内调用 `Change` 是安全的——请求会排队，在当前转换结束后按 FIFO 应用，不会产生递归 Exit/Enter 栈。
- **异常不再静默**：与旧版「Change 内无 try/catch」不同，现在状态回调异常被包成 `FrameStateMachineException` 抛出（带 InnerException），便于定位；但仍可能在异常时停在「引用已切、Enter 未完成」的中间态，状态实现应自行保证健壮。
- **enum id 性能**：用枚举作 id 时务必传 `EqualityComparer<TEnum>.Default`，否则字典查找会因默认比较器装箱产生 GC。
- **不自动 Tick**：状态机不持有更新循环，需调用方在 `Update` 调 `Tick`，并按需在 `FixedUpdate/LateUpdate` 调 `FixedTick/LateTick`。
- **Clear 后不可直接复用**：`Clear` 会清空注册表与转换表，需重新 `Add` 再 `Change`。
...
- **打开/关闭过渡（transition）**：通过 `IUITransition` 抽象，内置淡入淡出 `UIFadeTransition`。
...
| `UIFadeTransition`（`sealed class : IUITransition`） | 淡入淡出过渡 | 默认 duration=0.18，unscaledTime=true；操作 `CanvasGroup.alpha` |
...
内置淡入淡出过渡 `sealed class : IUITransition`，操作面板上的 `CanvasGroup.alpha`。

| 成员 | 作用 | 实现/注意点 |
| --- | --- | --- |
| `UIFadeTransition(float duration = 0.18f, bool unscaledTime = true)` | 构造 | 把 `duration` 同时赋给 `OpenDuration` 与 `CloseDuration`，`UseUnscaledTime=unscaledTime`（**默认 duration=0.18、unscaledTime=true**） |
...
Config 模块提供一个**多 Provider 链 + 缓存 + 校验**的配置加载体系。核心入口是 `ConfigService`（继承 `GameModuleBase`，`Priority = -200`，在初始化时把自身注册到 `Context.Services`）。配置通过 `Load<TConfig>(key)` / `TryLoad<TConfig>(key, out config)` 泛型方法按字符串 `key` + 目标类型读取，底层逐个询问已注册的 `IConfigProvider`。框架内置三类 Provider：基于 `Resources` 的 `ScriptableObject` 配置（`ResourcesScriptableConfigProvider`）、基于 `Resources` 的 JSON 文本配置（`ResourcesJsonConfigProvider`，默认根目录 `Configs`），以及运行时可写的远端/覆盖 JSON 配置（`RuntimeJsonConfigProvider`，同时实现 `IConfigChangeNotifier`，变更时自动失效缓存）。配置对象可选实现 `IConfigValidator`，加载后立即校验；校验失败则视为加载失败。JSON 反序列化使用 Newtonsoft.Json (`JsonConvert`)。
...
| `ResourcesJsonConfigProvider` | 从 `Resources` 读 JSON 文本资产 | 默认根目录 `Configs`；`Resources.Load<TextAsset>` + `JsonConvert.DeserializeObject` |
| `ResourcesScriptableConfigProvider` | 从 `Resources` 读 `ScriptableConfig` 资产 | 构造时 `Resources.LoadAll<ScriptableObject>` 全量加载并注册到内部 `ScriptableConfigProvider` |
...
| `ScriptableConfigProvider` | 内存中按 `Id` 索引的 `ScriptableConfig` 字典 | `Register`/`TryLoad`/`Clear`；被 Resources 版本复用 |
...
| `protected override void OnInitialize()` | 初始化钩子 | 依次 `providers.Add(new ResourcesScriptableConfigProvider())`、`providers.Add(new ResourcesJsonConfigProvider())`；随后 `Context.Services.Register<IConfigService>(this)` 与 `Register(this)`。注意：默认链中 ScriptableObject Provider 在前、JSON Provider 在后 |
...
### `ResourcesJsonConfigProvider.cs`

从 `Resources` 加载 JSON 文本资产（`TextAsset`）并反序列化，`sealed`，实现 `IConfigProvider`。

| 成员 | 作用 | 实现/注意点 |
| --- | --- | --- |
| `private readonly string rootPath` | Resources 下的根目录 | 构造时归一化保存 |
| `ResourcesJsonConfigProvider(string rootPath = "Configs")` | 构造函数 | **默认根目录 `"Configs"`**；`rootPath = FramePathUtility.NormalizeResourcesPath(rootPath)`（反斜杠转斜杠、去扩展名、剥离到 `/Resources/` 之后） |
| `bool TryLoad<TConfig>(string key, out TConfig config)` | 加载并反序列化 | ① 拼路径：`rootPath` 为空则 `path = key`，否则 `rootPath + "/" + key`，再 `NormalizeResourcesPath`；② `Resources.Load<TextAsset>(path)`，为 `null` 则 `config = null; return false`；③ `try { config = JsonConvert.DeserializeObject<TConfig>(textAsset.text); return config != null; }`；④ 异常时 `FrameLog.Exception(exception)`，`config = null; return false`。无变更通知 |

---

### `ResourcesScriptableConfigProvider.cs`

从 `Resources` 批量加载 `ScriptableConfig` 资产，`sealed`，实现 `IConfigProvider`。内部委托给一个 `ScriptableConfigProvider`。

| 成员 | 作用 | 实现/注意点 |
| --- | --- | --- |
| `private readonly ScriptableConfigProvider provider` | 内部内存索引 | `new ScriptableConfigProvider()` |
| `ResourcesScriptableConfigProvider(string rootPath = "Configs")` | 构造时全量加载 | **默认根目录 `"Configs"`**；`path = NormalizeResourcesPath(rootPath)`；`Resources.LoadAll<ScriptableObject>(path)` 遍历，凡 `as ScriptableConfig` 非空者 `provider.Register(config)`。注意：构造时即一次性加载，运行时新增资产不会自动感知 |
| `bool TryLoad<TConfig>(string key, out TConfig config)` | 透传查询 | 直接 `return provider.TryLoad(key, out config)`（按 `ScriptableConfig.Id` 匹配 key） |
...
内存中按 `Id` 索引 `ScriptableConfig` 的 Provider，`sealed`，实现 `IConfigProvider`。被 `ResourcesScriptableConfigProvider` 复用，也可单独手动注册。
...
1. **默认 Provider 链构建**：`ConfigService.OnInitialize` 按顺序加入两个内置 Provider —— 先 `ResourcesScriptableConfigProvider`（根目录 `Configs`，构造时 `Resources.LoadAll` 全量加载 `ScriptableConfig`），后 `ResourcesJsonConfigProvider`（根目录 `Configs`，`Resources.Load<TextAsset>` + Newtonsoft 反序列化）。链表遍历顺序即优先级，索引 0 最高，故默认情况下 `ScriptableObject` 配置优先于 JSON 文本配置。
2. **注册自定义 Provider**：`RegisterProvider` 用 `providers.Insert(0, provider)` **插入链首**，因此后注册者优先级更高（典型用法：注册 `RuntimeJsonConfigProvider` 作为远端/热更覆盖层，覆盖本地 `Resources` 配置）。每次注册都会 `ClearCache()`，并在 Provider 实现 `IConfigChangeNotifier` 时订阅其 `Changed`。
...
// 2) 加载 Resources/Configs/game.json（JSON Provider 反序列化为 POCO）
...
// 4) ScriptableObject 配置：定义并被 ResourcesScriptableConfigProvider 自动加载
public sealed class EnemyConfig : ScriptableConfig
{
    public int hp;
}
// 资产放在 Resources/Configs/ 下，Id 默认为资产名；
EnemyConfig enemy = configs.Load<EnemyConfig>("Goblin");
...
- **默认链顺序固定**：内置链是 `[ScriptableConfig, Json]`，即同名 key 下 `ScriptableObject` 配置会先命中。若希望 JSON 优先，需用 `RegisterProvider` 重新注册一个 JSON Provider 到链首。
...
- **键大小写**：`RuntimeJsonConfigProvider` 用 `OrdinalIgnoreCase` 且 key 会经 `NormalizeResourcesPath`（去扩展名、剥 `/Resources/` 前缀）；而 `ScriptableConfigProvider` 字典是大小写敏感的、键为 `Id`。跨 Provider 用同一 key 时要注意这点不一致。
- **`ResourcesScriptableConfigProvider` 构造即加载**：它在构造函数里 `Resources.LoadAll` 一次性扫描，运行时新增/删除资产不会反映；如需动态可改用 `RuntimeJsonConfigProvider` 或手动维护 `ScriptableConfigProvider`。
...
Networking 模块封装基于 `UnityWebRequest` 的 HTTP 客户端。核心是 `HttpService`（继承 `GameModuleBase`，`Priority = 0`，初始化时注册到 `Context.Services`），底层用 **UniTask**（`Cysharp.Threading.Tasks`）异步发送，JSON 序列化/反序列化用 Newtonsoft.Json。它提供便捷方法 `Get`/`PostJson` 与强类型 `GetJson<T>`/`PostJson<Req,Resp>`/`SendJson<T>`；强类型解析委托给可插拔的 `IHttpResponseParser`（默认裸 JSON 的 `JsonHttpResponseParser`，以及解析 `{code,message,data,success}` 信封的 `EnvelopeHttpResponseParser`）。每次请求返回 `HttpRequestHandle`（可 `yield` 等待，可 `Cancel`）。服务维护 `BaseUrl`（相对路径自动拼接）、默认请求头（含 Bearer Token）与一组指标计数器，并通过 `RequestStarted`/`RequestCompleted` 事件对外广播——这些计数器供诊断 Overlay 使用。
...
| `LocalizedTextTable`（ScriptableObject，sealed） | 文本数据表 | `key` 列 + 多 `locale` 列；支持 `TextAsset` 源或手动条目；惰性构建 `locale->key->value` 缓存 |
| `LocalizedTextTable.Entry`（class，sealed，私有） | 单条文本记录 | `[Serializable]`，`Key` + `Values`（与 locale 列一一对应） |
...
| 成员 | 作用 | 实现/注意点 |
| --- | --- | --- |
| `TextAsset source` | 表格源文件 | `[SerializeField]`，`[Header("Spreadsheet Source")]`；非空且非空文本时优先于手动条目 |
| `string delimiter = ","` | 分隔符 | `[SerializeField]`，默认 `","`；TSV 以字面 `"\\t"` 存储（两字符反斜杠+t），由 `ResolveDelimiter` 还原为 `'\t'` |
| `List<string> locales` | 手动语言列表 | `[SerializeField]`，`[Header("Manual Fallback")]`，默认 `{ "en" }` |
| `List<Entry> entries` | 手动条目列表 | `[SerializeField]`，默认空 |
...
| `void Clear()` | 清空全部 | 置 `source=null`、清 `locales`/`entries`、`MarkDirty()` |
| `void SetLocales(params string[] localeCodes)` | 设置语言列 | 清 `locales`，逐个 `AddLocaleIfMissing(CleanKey(...))`；`NormalizeEntryValueCounts()` 对齐各条目值数量；`MarkDirty()` |
| `void SetValue(string key, string locale, string value)` | 设置单元格 | `CleanKey` key/locale；空白则忽略；`AddLocaleIfMissing` 取列索引，`FindOrCreateEntry`，`EnsureValueCount`，写入 `entry.Values[localeIndex] = value ?? ""`；`MarkDirty()` |
...
| `void ImportDelimitedText(string text, char textDelimiter)` | 通用分隔文本导入 | 置 `source=null`；`delimiter` 记为 `"\\t"`（制表符）或字符本身；清 `locales`/`entries`；`ParseDelimited` 后 `ApplyRows` 写入手动条目；`MarkDirty()` |
| `void SetSource(TextAsset textAsset, string textDelimiter = ",")` | 设置表格源 | `source=textAsset`；`delimiter` 空则 `","` 否则取传入值；`MarkDirty()` |
| `void OnEnable()` | 启用回调 | 私有；置 `lookupDirty=true` |
| `void OnValidate()` | 编辑器校验 | 私有，`#if UNITY_EDITOR`；置 `lookupDirty=true` |
| `void EnsureLookup()` | 构建/复用缓存 | 私有；不脏且 `lookup!=null` 直接返回；否则新建/清空字典、清 `availableLocales`；**`source!=null && source.text` 非空 → `BuildLookupFromRows(ParseDelimited(source.text, ResolveDelimiter()))`，否则 `BuildLookupFromSerializedEntries()`**；末尾置 `lookupDirty=false` |
| `void BuildLookupFromSerializedEntries()` | 从手动条目建缓存 | 私有；遍历 `entries`（跳过 `null`/空 Key），对每个 `locales[j]`（`CleanKey` 后非空白）取 `entry.Values[j]`（越界用 `""`），`AddLookupValue(locale, entry.Key, value)` |
...
| `void ApplyRows(List<List<string>> rows)` | 表格行写回手动条目 | 私有；空集或首行列数 <2 返回；表头第 2 列起 `AddLocaleIfMissing`；数据行 `FindOrCreateEntry`、`EnsureValueCount`，按 `columnIndex-1` 写入 `entry.Values`（受 `locales.Count` 上限约束） |
...
| `int AddLocaleIfMissing(string locale)` | 取/追加语言列索引 | 私有；空白返回 `-1`；已存在返回其索引，否则追加并返回新索引 |
| `Entry FindOrCreateEntry(string key)` | 取/建条目 | 私有；按 Key 线性查找，找不到则 `new Entry { Key = key }` 追加 |
| `void NormalizeEntryValueCounts()` | 对齐各条目值数量 | 私有；对每个非空条目 `EnsureValueCount(Values, locales.Count)` |
| `static void EnsureValueCount(List<string> values, int count)` | 补齐值列表 | 私有；不足 `count` 时补 `string.Empty` |
...
| `Entry`（嵌套类） | 单条记录 | `[Serializable] private sealed class`；`string Key = ""`、`List<string> Values = new()` |
...
- 数据来源二选一：若 `source`（TextAsset）非空且有文本，按 `ResolveDelimiter()` 解析其内容；否则用序列化的手动 `locales`/`entries`。
...
- **表头列数必须 ≥2**：`BuildLookupFromRows`/`ApplyRows` 对列数 <2 的表头直接放弃（至少要有 key 列 + 1 个 locale 列）。
...
| `TweenOptions`（class，sealed） | 补间可选配置 | `Ease`（默认 `OutQuad`）、`IgnoreTimeScale`、`Target`、`Completed` |
...
> 注意：源码中 `TweenOptions` **不存在** `Loops`、`Delay`、`LoopType` 等字段——本抽象只暴露 `Ease / IgnoreTimeScale / Target / Completed` 四项。
...
- **抽象不含循环/延迟**：`TweenOptions` 只有 `Ease/IgnoreTimeScale/Target/Completed` 四项，**没有 `Loops`、`Delay`、`LoopType`**。需要循环或延迟必须自行用句柄编排或改用 DOTween 实现层（会破坏抽象隔离，谨慎）。
...
| `public ITweenHandle Fade(CanvasGroup target, float endValue, float duration, TweenOptions options = null)` | 透明度补间 | `target == null` → 空句柄；否则 `DOTween.To(() => target.alpha, v => target.alpha = v, Mathf.Clamp01(endValue), Mathf.Max(0f,duration))`（endValue 钳到 [0,1]）→ `SetTarget(target)` → `ApplyOptions` → 包装 |
...
| `private static void ApplyOptions(Tween tween, TweenOptions options)` | 应用补间选项 | `tween == null` 直接返回；`options ?? new TweenOptions()` 取默认；`tween.SetEase(MapEase(resolved.Ease))`；`tween.SetUpdate(resolved.IgnoreTimeScale)`（true 则忽略 TimeScale）；`resolved.Target != null` → `tween.SetTarget(Target)`（覆盖默认目标，便于按业务对象 `Kill`）；`resolved.Completed != null` → `tween.OnComplete(() => resolved.Completed())` |
...
`RegisterProvider` 把新 provider **插到链头**，因此它的配置会覆盖默认 `ResourcesScriptableConfigProvider` / `ResourcesJsonConfigProvider`。配置对象实现 `IConfigValidator` 时，加载后会自动校验，失败返回 null/false 并写框架 warning。

---

## 28. 生产化检查表（速查）

> 详细论述见 `FRAMEWORK_GUIDE.md`。这里只做上线前快速核对。
...
- **网络**：确认统一 `ResponseParser`、鉴权刷新、错误码动作映射、重试与埋点。
...
| Networking | `IHttpService` | `HttpService` | 0 | `Frame.Networking` |
